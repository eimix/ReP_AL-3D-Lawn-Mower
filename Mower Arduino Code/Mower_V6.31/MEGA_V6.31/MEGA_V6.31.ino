...
int PWM_MaxSpeed_LH            = 255; //240     //EEPROM            // Straight line speed LH Wheel (Looking from back of mower)  Will be overidden if saved in EEPROM
...
void loop()
{
  ...
  if (Mower_SaftyWarning == 1) Print_Mower_Warning();                                     // Safety(pause) mode incase the mower in an warning state
  if (Mower_Error == 1) Print_Mower_Error();                                              // Safety mode incase the mower is lost or in an error state
